...
def max3(x, y, z):  #function to find the maximum of three numbers
    if x >= y and x >= z:
        return x
    elif y >= x and y >= z:
        return y
...
def optimal_alignments(alignment_a, alignment_b, i, j):     
    ...
    if i == 0 and j == 0:   
        print(alignment_a[::-1])        # print the alignment_a string after reversing it because the string is built from the end (bottom right corner of the matrix)
        print(alignment_b[::-1])        # print the alignment_b string after reversing it because the string is built from the end (bottom right corner of the matrix)
        print("Score:", Score[la][lb])   # print the score of the alignment
        print()
        return
...
# initialize the first row and first column of the score matrix with gap penalties
for i in range(la+1):
    Score[i][0] = -(i * gap)
...
for i in range(la):
    for j in range(lb):
        ...
        final_value = max3(t0, t1, t2)  # find the maximum value among the three values
